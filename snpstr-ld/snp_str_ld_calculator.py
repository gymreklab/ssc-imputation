...
def CalcLD(str_reader, snp_reader, str_locus, snp_locus, use_info_start=False):
...
def main():
    ...
    if args.str_locus:
        ...
        if args.snp_locus:
            ...
            snpid = snp_locus
            ld = CalcLD(str_reader, snp_reader, str_locus, snp_locus, use_info_start=args.use_info_start)
            ...
            snp_locus = FindSnpLocus(snp_reader, args.str_locus, args.snp_locus_rsid, snp_region_start, snp_region_end)
    ...
    if args.loci_file_rsid is not None:
        with open(args.loci_file_rsid, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    try:
                        ...
                        snp_region_end = int(items[5])
                    snp_locus = FindSnpLocus(snp_reader, str_locus, rsid, snp_region_start, snp_region_end)
                    snp_rsid_to_pos[rsid] = snp_locus
                if snp_locus is None:
                    sys.stderr.write("Could not find %s\n"%rsid)
                    continue
                ld = CalcLD(str_reader, snp_reader, str_locus, snp_locus, use_info_start=args.use_info_start)
                PrintLine(str_locus, rsid, ld)
